...
import { getAccountabilityScorecards } from 'apiSdk/accountability-scorecards';
import { AccountabilityScorecardInterface } from 'interfaces/accountability-scorecard';
import { getUserPerformances } from 'apiSdk/user-performances';
import { UserPerformanceInterface } from 'interfaces/user-performance';
import { getRegularMeetings } from 'apiSdk/regular-meetings';
import { RegularMeetingsInterface } from 'interfaces/regular-meetings';

import { RegularMeetingsListPage as ShowCaseList } from 'pages/regular-meetings';

function DashboardPage() {
  const { session } = useSession();

  const accountabilityScorecardStat = useSWR<PaginatedInterface<AccountabilityScorecardInterface>>(
    () => `/accountability-scorecards`,
    () => getAccountabilityScorecards({ limit: 1, offset: 0 }),
  );

  const userPerformanceStat = useSWR<PaginatedInterface<UserPerformanceInterface>>(
    () => `/user-performances`,
    () => getUserPerformances({ limit: 1, offset: 0 }),
  );

  const regularMeetingsStat = useSWR<PaginatedInterface<RegularMeetingsInterface>>(
    () => `/regular-meetings`,
    () => getRegularMeetings({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Accountability Scorecards',
      result: accountabilityScorecardStat?.data?.totalCount,
      isLoading: accountabilityScorecardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Performances',
      result: userPerformanceStat?.data?.totalCount,
      isLoading: userPerformanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Regular Meetings',
      result: regularMeetingsStat?.data?.totalCount,
      isLoading: regularMeetingsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Regular Meetings
                </Text>
                <NextLink href={`regular-meetings`} passHref legacyBehavior>
                  <Button
